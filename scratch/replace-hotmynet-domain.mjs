...
function cleanString(val) {
  ...
  let nextVal = val;
  // Replace absolute hotmynet.shop URLs with relative ones
  if (nextVal.includes('hotmynet.shop')) {
    nextVal = nextVal.replace(/https?:\/\/(www\.)?hotmynet\.shop/g, '');
    console.log(`  Cleaned domain: "${val}" -> "${nextVal}"`);
  }
  return nextVal;
}
