...
async function main() {
  ...
  console.log('--- Verification of saved product edits in database ---');
  if (!products) {
  ...
  for (const [id, prod] of Object.entries(products)) {
    if (prod.title === 'MOM SON' || prod.title === 'SIS BRO' || prod.slug === 'mom-son' || prod.slug === 'sis-bro') {
      console.log(`Product ID: ${id}`);
      console.log(`Title:      ${prod.title}`);
      console.log(`Slug:       ${prod.slug}`);
      console.log(`Display Ord:${prod.displayOrder}`);
      console.log(`Image URL:  ${prod.image}`);
      console.log('----------------------------------------------------');
    }
  }
}
